...
if(!empty($post['action']) && $post['action']=="create") {
	...
	$event->setCompanyName($post['company_name']);
    $event->setImage($post['image']);
	$event->setText($post['text']);
        ...
        <div class="row align-items-center">
          <div class="col-md-12 col-md-right">
           <div class="form-group">
              <div class="col-sm-12">          
                <input type="text" class="form-control" id="event-name" placeholder="Имя" name="name" value="'.$fetchEvent['name'].'">
              </div>
            </div>
            <div class="form-group">
              <div class="col-sm-12">
                <textarea class="form-control" id="event-image'.$fetchEvent['id'].'" name="company_name">'.$fetchEvent['company_name'].'</textarea>
              </div>
            </div>
            <div class="form-group">
              <div class="col-sm-12">
                <textarea class="form-control" id="event-image'.$fetchEvent['id'].'" name="image">'.$fetchEvent['image'].'</textarea>
              </div>
            </div>
            <div class="form-group">
              <div class="col-sm-12">
                <textarea class="form-control" id="event-content'.$fetchEvent['id'].'" name="text">'.$fetchEvent['text'].'</textarea>
              </div>
            </div>
            <div class="form-group">
              <div class="col-sm-12">       
