...
if(!empty($post['action']) && $post['action']=="create") {
    $event->setNameVacancy($post['name_vacancy']);
	$event->setVacancy($post['vacancy']);
	...
	echo '<div class="card gedf-card" style="margin: 5px;" id="dyn-'.$status.'">
      <div class="card-body">
          <p class="card-text">'.$post['name_vacancy'].'</p>
          <p class="card-text">'.$post['vacancy'].'</p>
	...
	<input type="hidden" name="action" value="update">
        ...
        <div class="row align-items-center">
          ...
          <div class="form-group">
              <div class="col-sm-12">          
                <input type="text" class="form-control" id="event-name" placeholder="Название вакансии" name="name_vacancy" value="'.$fetchEvent['name_vacancy'].'">
              </div>
            </div>
            <div class="form-group">
              <div class="col-sm-12">
                <textarea class="form-control" id="event-content'.$fetchEvent['id'].'" name="vacancy">'.$fetchEvent['vacancy'].'</textarea>
              </div>
            </div>
            <div class="form-group">
              <div class="col-sm-12">       
